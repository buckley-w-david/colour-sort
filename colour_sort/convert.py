...
class ConverterType(str, Enum):
    ...
    @staticmethod
    def to_converter(converter_type: 'ConverterType'):
        if converter_type is ConverterType.PIL:
            return PilConverter()
        elif converter_type is ConverterType.CV2:
            return Cv2Converter()
        else:
            return SkimageConverter()
...
class PilConverter:
    ...
    def rgb2lab(self, image: np.ndarray) -> np.ndarray:
        ...
        rgb2lab = ImageCms.buildTransformFromOpenProfiles(
            PilConverter.__srgb_p, PilConverter.__lab_p, "RGB", "LAB"
        )
    ...
    def lab2rgb(self, image: np.ndarray) -> np.ndarray:
        ...
        rgb2lab = ImageCms.buildTransformFromOpenProfiles(
            PilConverter.__lab_p, PilConverter.__srgb_p, "LAB", "RGB"
        )
        return np.array(ImageCms.applyTransform(im, rgb2lab))
...
def convert(
    image: np.ndarray,
    converter: ColourSpaceConverter,
    source_type: ColourSpace,
    result_type: ColourSpace,
):
