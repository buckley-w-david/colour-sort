...
SHIFT_ORDERS = {
    MethodType.ABC: [0, 1, 2],
    MethodType.ACB: [0, 2, 1],
    MethodType.CBA: [2, 1, 0],
    MethodType.CAB: [2, 0, 1],
    MethodType.BAC: [1, 0, 2],
    MethodType.BCA: [1, 2, 0],
    MethodType.ABCC: [0, 1, 2],
    MethodType.ACBC: [0, 2, 1],
    MethodType.CBAC: [2, 1, 0],
    MethodType.CABC: [2, 0, 1],
    MethodType.BACC: [1, 0, 2],
    MethodType.BCAC: [1, 2, 0],
}
...
def _reshape_image(image):
    res = cv2.resize(
        np.array(image), dsize=(4096, 4096), interpolation=cv2.INTER_LANCZOS4
    )
...
def create_sorted_image(
    image: Image.Image,
    method: MethodType,
    colour_space: ColourSpace,
    converter: convert.ColourSpaceConverter,
) -> Image.Image:
    ...
    if method is MethodType.BRIGHTNESS:
        # Must be in LAB
        if colour_space is not ColourSpace.LAB:
            logging.warning(
                "Colour space needs to be in LAB to correctly sort by brightness"
            )
            colour_space = ColourSpace.LAB
        reduce_func = partial(reducer.reduce_shift, order=[0])
    elif method is MethodType.AVG:
        reduce_func = reducer.reduce_sum
    elif method is MethodType.ZORDER:
        reduce_func = reducer.reduce_z_order
    # elif mode is MethodType.HILBERT:
    #     reduce_func = reducer.reduce_hilbert
    elif method is MethodType.PCA:
        reduce_func = reducer.reduce_pca
    else:
        reduce_func = partial(reducer.reduce_shift, order=SHIFT_ORDERS[method])
    ...
    reshaped = _reshape_image_old(
        Image.fromarray(normalized_image, mode=colour_space.name)
    )

    # If we don't want to intentially allow overflow, use uint32 so there's enough room for the bit shifting
    if not method.is_clip():
    ...
    return Image.fromarray(
        np.reshape(sorted_image.astype(np.uint8), (IMAGE_SIZE, IMAGE_SIZE, 3)),
        mode=ColourSpace.RGB.name,
    )
